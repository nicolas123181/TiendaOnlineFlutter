...
final productSizesStockProvider = FutureProvider.family<List<ProductSizeStock>, int>((ref, productId) async {
...
final sizeStockProvider = FutureProvider.family<int, ({int productId, String size})>((ref, params) async {
  final sizes = await ref.watch(productSizesStockProvider(params.productId).future);
  final sizeData = sizes.firstWhere(
    (s) => s.size.toUpperCase() == params.size.toUpperCase(),
    orElse: () => ProductSizeStock(id: 0, productId: params.productId, size: params.size, stock: 0),
  );
  return sizeData.stock;
});

/// Provider para obtener el mapa de tallas y stock
final productSizesMapProvider = FutureProvider.family<Map<String, int>, int>((ref, productId) async {
